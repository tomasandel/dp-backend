...
Express API that collects Signed Tree Heads (STHs) from CT monitors and exposes them for cross-monitor consistency checks. Acts as the central data store for the CT verification system.
...
- Provides aggregate statistics (per-log breakdowns, ingestion rates, consistency checks)
...
| `GET /api/stats` | Aggregate statistics and cross-monitor consistency |
...
src/
  ...
  routes/
    ...
    stats.ts            # Statistics and consistency endpoint
